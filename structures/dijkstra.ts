...
┌────────────────────────────────────────────────────────┐
│ Инициализация:                                         │
│ • Стартовая вершина: метка = 0                         │
│ • Остальные вершины: метка = ∞ (бесконечность)         │
...
└────────────────────────────────────────────────────────┘
      ...
      ▼                                  │
    / Есть ли ещё \                      │
   <  непосещённые > ───(Нет)───► [ Конец: кратчайшие пути найдены ]
    \   вершины?  /                      │
      │                                  │
    (Да)                                 │
      │                                  │
      ▼                                  │
┌──────────────────────────────────────┐ │
│ Выбрать НЕПОСЕЩЁННУЮ вершину (U)     │ │
│ с минимальной текущей меткой         │ │
└──────────────────────────────────────┘ │
      │                                  │
      ▼                                  │
┌──────────────────────────────────────┐ │
│ Отметить вершину U как ПОСЕЩЁННУЮ    │ │
│ (её кратчайший путь уже найден)      │ │
└──────────────────────────────────────┘ │
      │                                  │
      ▼                                  │
    / Перебрать всех \                   │
   < непосещённых соседей > ─(Все)───────┘
    \  вершины U (V) /
      │
    (След. сосед)
      │
      ▼
    / Новое расстояние до V через U     \
   <  меньше, чем текущая метка у V?     >
    \ (Метка U + Вес рёбра U→V < Метка V) /
      │
    (Да)
      │
      ▼
┌──────────────────────────────────────┐
│ Релаксация (обновление):             │
│ • Перезаписать метку V новым числом  │
│ • Запомнить, что в V пришли из U     │
└──────────────────────────────────────┘
      │
      ▼
      └──────────────────────────────────► Назад к проверке соседей
